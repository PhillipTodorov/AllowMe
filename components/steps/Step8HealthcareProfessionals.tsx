...
const inputClass = (hasError: boolean) =>
  `w-full border rounded-lg px-3 py-2 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm ${
      ...
      {fields.map((field, index) => (
        <div key={field.id} className="mb-4 p-5 bg-white border border-gray-200 rounded-xl shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-semibold text-gray-700">Professional {index + 1}</h3>
            {fields.length > 1 && (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              {errors.professionals?.[index]?.name && <p className="text-xs text-red-600 mt-1">{errors.professionals[index]?.name?.message}</p>}
            </div>
            <div>
              ...
              {errors.professionals?.[index]?.role && <p className="text-xs text-red-600 mt-1">{errors.professionals[index]?.role?.message}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mt-3">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('lastSeen')}</label>
              <input {...register(`professionals.${index}.lastSeen`)} className={inputClass(false)} placeholder="e.g. January 2024" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('treatedFor')}</label>
